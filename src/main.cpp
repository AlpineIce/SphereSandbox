...
int main()
{
	...
	std::vector<std::unique_ptr<Actor::PhysicalActor>> spheres(500);
	...
	while (!exitLoop)
	{
		...
		modelTransform.location = glm::vec3((float)sin(*engine.time) * 3.0f, 4.0f, (float)cos(*engine.time) * 3.0f);
		modelTransform.rotation = glm::quat((float)sin(*engine.time), 0.0f, (float)cos(*engine.time), 0.0f);
		testModel->transform(modelTransform);

		Actor::Transformation model2Transform;
		model2Transform.location = glm::vec3((float)-sin(*engine.time) * 3.0f, 4.0f, (float)-cos(*engine.time) * 3.0f);
		model2Transform.rotation = glm::quat((float)sin(*engine.time * 0.2f), 0.0f, (float)cos(*engine.time * 0.2f), 0.0f);
